...
public String getSalaryEmploye(
    ...
    public void exportSalaryPdf(String name, HttpSession session, HttpServletResponse response)
        ...
        // ✅ CORRECTION 2: Supprimer la variable inutilisée 'white'
    ...
    public String genererSalaire(
        ...
        // Si la checkbox "useMoyenne" est cochée, calculer la moyenne
        if ("true".equals(useMoyenne)) {
            double averageSalary = baseSalaryService.getAverageBaseSalary(session);
            if (averageSalary > 0) {
                montant = averageSalary;
                System.out.println("🔢 Utilisation de la moyenne des salaires: " + montant);
            } else {
                redirectAttributes.addFlashAttribute("error", "❌ Impossible de calculer la moyenne des salaires.");
        ...
        if (success) {
            String message = "✅ Salaire généré avec succès.";
            if ("true".equals(useMoyenne)) {
                message += " (Moyenne utilisée: " + String.format("%.2f", montant) + ")";
            }
    ...
    public String RecherchesSalary(@RequestParam("component") String componentName,
        ...
        return "recherche-salaire"; // Pas de "redirect" sinon on perd les données
    }
